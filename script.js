...
function update3DScene(weatherCode) {
    ...
    if ((weatherCode >= 51 && weatherCode <= 67) || (weatherCode >= 80 && weatherCode <= 82)) {
        ...
        const rainVertices = [];
        for (let i = 0; i < 5000; i++) { rainVertices.push(Math.random() * 10 - 5, Math.random() * 10 - 5, Math.random() * 10 - 5); }
        ...
        const snowVertices = [];
        for (let i = 0; i < 5000; i++) { snowVertices.push(Math.random() * 10 - 5, Math.random() * 10 - 5, Math.random() * 10 - 5); }
    ...
    if (weatherCode <= 1) { sun.intensity = 2.5; clouds.material.opacity = 0.2; }
    else if (weatherCode <= 3) { sun.intensity = 1.0; clouds.material.opacity = 0.6; }
    else { sun.intensity = 0.5; clouds.material.opacity = 0.8; }
}
...
async function getWeatherData(latitude, longitude, cityName) {
    // UPDATED: Added precipitation_probability to the hourly parameters
...
function updateUI(data, cityName) {
    ...
    const dailyRainChance = daily.precipitation_probability_max[0];
    document.getElementById('rain-chance').innerHTML = `${dailyRainChance} %`;
    const weatherEmojiEl = document.getElementById('weather-emoji');
    if (dailyRainChance > 50 || current.weather_code >= 51) {
    ...
    for (let i = startIndex; i < startIndex + 24 && i < hourly.time.length; i++) {
        ...
        // NEW: Get hourly rain probability
        const hourlyRainProb = hourly.precipitation_probability[i];

        const hourDiv = document.createElement('div');
        hourDiv.className = 'flex-shrink-0 text-center p-3 rounded-xl bg-white/10 w-28 glass-panel'; // Increased width to w-28
        // UPDATED: Added hourly rain chance to the innerHTML
        hourDiv.innerHTML = `
            <p class="font-medium">${time.getHours()}:00</p>
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    const displayName = admin1 ? `${name}, ${admin1}` : `${name}, ${country}`;
                    getWeatherData(latitude, longitude, displayName);
                } else { getWeatherData(latitude, longitude, "Your Location"); }
            } catch { getWeatherData(latitude, longitude, "Your Location"); }
        }, (error) => {
